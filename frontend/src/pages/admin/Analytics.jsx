...
import {
  ...
  LineChart, Line, PieChart, Pie, Cell, Legend, Brush, ReferenceLine,
} from 'recharts';
import {
  getCategoryDistribution, getMonthlyTrends,
  getAreaDistribution, getOfficerPerformance,
...
// Lightweight animated bar skeleton shown while each chart loads
function ChartSkeleton() {
  return (
    <div className="h-64 flex items-end gap-2 px-4 pb-4 animate-pulse">
...
function TableSkeleton() {
  return (
    <div className="animate-pulse space-y-3 p-2">
      {[1, 2, 3].map(i => (
        <div key={i} className="h-8 bg-slate-200 rounded-md" />
      ))}
    </div>
  );
}
...
export default function Analytics() {
  ...
  // Per-section loading — page renders immediately, sections fill in as data arrives
  const [loadingCharts, setLoadingCharts] = useState(true);
  const [loadingPerf, setLoadingPerf] = useState(true);

  useEffect(() => {
    // All 4 endpoints fire in parallel — no sequential waiting
    const fetchAll = async () => {
      const [catRes, trendRes, areaRes, perfRes] = await Promise.allSettled([
      ...
      if (perfRes.status === 'fulfilled') {
        ...
        setPerformance(d?.success ? (d.data || []) : []);
      }
      setLoadingPerf(false);

      const failCount = [catRes, trendRes, areaRes, perfRes].filter(r => r.status === 'rejected').length;
      if (failCount === 4) toast.error('Failed to load analytics data');
    };
  ...
  const trendSummary = useMemo(() => {
    ...
    return { month: latest.month, total, resolved, rate, delta };
  }, [filteredTrends]);
  ...
  const chartTrends = useMemo(() => {
    ...
    return [{ month: previousMonthLabel, total: 0, resolved: 0 }, onlyPoint];
  }, [filteredTrends]);
    ...
    <div className="slide-up">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="card p-6">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {['6m', '12m', 'all'].map(r => (
                <button
                  ...
                  className={`px-2.5 py-1 text-xs rounded-md border ${trendRange === r ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
                >
            ...
            <div className="mb-4 grid grid-cols-1 sm:grid-cols-3 gap-2">
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                ...
                <p className={`text-sm font-semibold ${trendSummary.delta >= 0 ? 'text-amber-600' : 'text-emerald-600'}`}>
                  {trendSummary.delta >= 0 ? '+' : ''}{trendSummary.delta} vs prev
                </p>
              </div>
            </div>
          )}
          ...
          <div className="h-64">
            {loadingCharts ? <ChartSkeleton /> : chartTrends.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartTrends} onClick={state => { if (state?.activeLabel) setSelectedTrendMonth(state.activeLabel); }}>
                  <CartesianGrid strokeDasharray="3 3" stroke={isDark ? '#334155' : '#f1f5f9'} />
                  <XAxis dataKey="month" tick={{ fontSize: 12, fill: '#94a3b8' }} />
                  <YAxis tick={{ fontSize: 12, fill: '#94a3b8' }} />
                  <Tooltip
                    contentStyle={{ borderRadius: '0.5rem', border: `1px solid ${isDark ? '#334155' : '#e2e8f0'}`, backgroundColor: isDark ? '#0f172a' : '#fff' }}
                    labelStyle={{ color: isDark ? '#e2e8f0' : '#334155', fontWeight: 600 }}
                  />
                  {showTotal && <Line type="monotone" dataKey="total" stroke="#6366f1" strokeWidth={2.5} dot={{ r: 3 }} activeDot={{ r: 6 }} name="Total" />}
                  {showResolved && <Line type="monotone" dataKey="resolved" stroke="#10b981" strokeWidth={2.5} dot={{ r: 3 }} activeDot={{ r: 6 }} name="Resolved" />}
                  {selectedTrendMonth && <ReferenceLine x={selectedTrendMonth} stroke="#94a3b8" strokeDasharray="4 4" />}
                  <Brush dataKey="month" height={22} stroke={isDark ? '#475569' : '#94a3b8'} travellerWidth={8} />
                </LineChart>
              </ResponsiveContainer>
            ) : (
        ...
        <div className="card p-6">
          ...
          <div className="h-64">
            {loadingCharts ? <ChartSkeleton /> : categories.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
        ...
        <div className="card p-6">
          ...
          <div className="h-64">
            {loadingCharts ? <ChartSkeleton /> : areas.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="card p-6">
          ...
          {loadingPerf ? <TableSkeleton /> : performance.length > 0 ? (
